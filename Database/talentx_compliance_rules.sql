...
CREATE TABLE `compliance_rules` (
  `compliance_rule_id` bigint NOT NULL AUTO_INCREMENT,
  `jurisdiction_id` bigint NOT NULL,
  `rule_category` enum('WORKING_HOURS','OVERTIME','MINIMUM_WAGE','LEAVE_ENTITLEMENT','SAFETY','DATA_PRIVACY','DISCRIMINATION','OTHER') COLLATE utf8mb4_unicode_ci DEFAULT NULL,
  ...
  KEY `idx_rule_jurisdiction` (`jurisdiction_id`),
  KEY `idx_rule_category` (`rule_category`),
  KEY `idx_rule_effective` (`effective_date`),
  CONSTRAINT `fk_compliance_rule_jurisdiction` FOREIGN KEY (`jurisdiction_id`) REFERENCES `compliance_jurisdictions` (`jurisdiction_id`) ON DELETE CASCADE
