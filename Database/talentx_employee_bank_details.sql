...
CREATE TABLE `employee_bank_details` (
  `employee_bank_detail_id` bigint NOT NULL AUTO_INCREMENT,
  `employee_id` bigint NOT NULL,
  `bank_name` varchar(255) NOT NULL,
  `account_number` varchar(50) NOT NULL,
  `ifsc_code` varchar(20) DEFAULT NULL,
  `branch_name` varchar(255) DEFAULT NULL,
  `account_type` enum('SAVINGS','CURRENT','SALARY') DEFAULT NULL,
  `is_primary` tinyint(1) DEFAULT '1',
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`employee_bank_detail_id`),
  UNIQUE KEY `uk_emp_bank` (`employee_id`,`account_number`),
  UNIQUE KEY `UK31ramrkq4srxx7kjsvufrxdm2` (`employee_id`,`account_number`),
